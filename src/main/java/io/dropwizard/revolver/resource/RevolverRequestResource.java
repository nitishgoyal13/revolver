...
public class RevolverRequestResource {
    ...
    public RevolverRequestResource(ObjectMapper jsonObjectMapper, ObjectMapper msgPackObjectMapper,
            PersistenceProvider persistenceProvider, InlineCallbackHandler callbackHandler, MetricRegistry metrics,
    ...
    private Response processRequest(String service, RevolverHttpApiConfig.RequestMethod method, String path,
    ...
    private String getPathFromHeaderExpression(RevolverHttpApiSplitConfig revolverHttpApiSplitConfig, String path,
            HttpHeaders headers) {
    ...
    private Response executeInline(String service, RevolverHttpApiConfig api,
            RevolverHttpApiConfig.RequestMethod method, String path, HttpHeaders headers, UriInfo uriInfo, byte[] body)
    ...
    private Response transform(HttpHeaders headers, RevolverHttpResponse response, String api, String path,
        ...
        String responseMediaType = response.getHeaders() != null && Strings.isNullOrEmpty(response.getHeaders()
                .getFirst(HttpHeaders.CONTENT_TYPE)) ? MediaType.TEXT_HTML : response.getHeaders()
                .getFirst(HttpHeaders.CONTENT_TYPE);
        String requestMediaType =
                headers != null && Strings.isNullOrEmpty(headers.getHeaderString(HttpHeaders.ACCEPT)) ? null
        ...
        if (Strings.isNullOrEmpty(requestMediaType) || requestMediaType.startsWith(MediaType.MEDIA_TYPE_WILDCARD)
    ...
    private Response executeCommandAsync(String service, RevolverHttpApiConfig api,
            RevolverHttpApiConfig.RequestMethod method, String path, HttpHeaders headers, UriInfo uriInfo, byte[] body,
            boolean isDownstreamAsync, String callMode) throws Exception {
        val sanatizedHeaders = new MultivaluedHashMap<String, String>();
        headers.getRequestHeaders()
                .forEach(sanatizedHeaders::put);
        cleanHeaders(sanatizedHeaders, api);
        ...
        if (persistenceProvider.exists(requestId)) {
            return Response.status(Response.Status.NOT_ACCEPTABLE)
                    .entity(ResponseTransformationUtil.transform(DUPLICATE_REQUEST_RESPONSE,
                            headers.getMediaType() == null ? MediaType.APPLICATION_JSON : headers.getMediaType()
                                    .toString(), jsonObjectMapper, msgPackObjectMapper))
                    .build();
        }
        persistenceProvider.saveRequest(requestId, mailBoxId, mailBoxAuthId, RevolverCallbackRequest.builder()
                .api(api.getApi())
        ...
        if (isDownstreamAsync) {
            ...
            return Response.accepted()
                    .entity(ResponseTransformationUtil.transform(revolverAckMessage,
                            headers.getMediaType() == null ? MediaType.APPLICATION_JSON : headers.getMediaType()
                                    .toString(), jsonObjectMapper, msgPackObjectMapper))
